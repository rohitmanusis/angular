...
( function(window) {'use strict';
		...
		ng.moveSelectionHandler = function($scope, elm, evt, grid) {
			...
			var charCode = evt.which || evt.keyCode, newColumnIndex, lastInRow = false, firstInRow = false, rowIndex = $scope.selectionService.lastClickedRow.rowIndex;
		...
		ngGridServices.factory('$domUtilityService', ['$utilityService',
		function($utils) {
			...
			domUtilityService.BuildStyles = function($scope, grid, digest) {
				...
				for (var i = 0; i < cols.length; i++) {
					...
					if (col.visible !== false) {
						...
						css += "." + gridId + " .col" + i + " { width: " + col.width + "px; left: " + colLeft + "px; height: " + rowHeight + "px }" + "." + gridId + " .colt" + i + " { width: " + col.width + "px; }";
						sumWidth += col.width;
					}
				};
				if ($utils.isIe) {// IE
		...
		ngGridServices.factory('$sortService', ['$parse',
		function($parse) {
			var sortService = {};
			...
			sortService.sortData = function(sortInfo, data /*datasource*/) {
				...
				var l = sortInfo.fields.length, order = sortInfo.fields, col, direction,
				...
				data.sort(function(itemA, itemB) {
					...
					while (tem == 0 && indx < l) {
						...
						direction = sortInfo.directions[indx], sortFn = sortService.getSortFn(col, d);
		...
		ngGridServices.factory('$utilityService', ['$parse',
		function($parse) {
			var utils = {
		...
		ng.Column = function(config, $scope, grid, domUtilityService, $templateCache, $utils) {
			...
			self.sort = function(evt) {
				if (!self.sortable) {
					return true;
					// column sorting is disabled, do nothing
				}
				...
				if ($scope.selectionProvider.lastClickedRow) {
					...
					if (columnIndex != undefined && elm) {
						...
						if (grid.config.showSelectionCheckbox && angular.element(columns[i]).scope() && angular.element(columns[i]).scope().col.index == 0) {
							i = 1;
							//don't want to focus on checkbox
						}
						if (columns[i]) {
		...
		ng.EventProvider = function(grid, $scope, domUtilityService) {
			...
			self.setDraggables = function() {
				if (!grid.config.jqueryUIDraggable) {
					...
					//have to iterate if using addEventListener
			...
			self.onRowMouseDown = function(event) {
				...
				if (rowScope) {
					...
					domUtilityService.eventStorage.rowToMove = {
						targetRow : targetRow,
						scope : rowScope
					};
				}
		...
		ng.Grid = function($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout) {
			...
			self.$root = null;
			//this is the root element that is passed in with the binding handler
			...
			if ( typeof self.config.data == "object") {
				...
				// we cannot watch for updates if you don't pass the string name
			}
				...
				if (asterisksArray.length > 0) {
					...
					angular.forEach(asterisksArray, function(col) {
						...
						if (col.index + 1 == numOfCols) {
							var offset = 2;
							//We're going to remove 2 px so we won't overlflow the viwport by default
							// are we overflowing?
		...
		ng.RowFactory = function(grid, $scope, domUtilityService, $templateCache, $utils) {
			...
			self.aggCache = {};
			self.parentCache = [];
			// Used for grouping and is cleared each time groups are calulated.
			...
			self.buildAggregateRow = function(aggEntity, rowIndex) {
				...
				// first check to see if we've already built it
				if (!agg) {
					// build the row
		...
		ng.selectionProvider = function(grid, $scope) {
			...
			// flag to prevent circular event loops keeping single-select var in sync
		...
		ng.StyleProvider = function($scope, grid, domUtilityService) {
			...
			$scope.headerStyle = function() {
				return {
					"width" : (grid.rootDim.outerWidth - domUtilityService.ScrollW) + "px",
					"height" : grid.config.headerRowHeight + "px"
				};
			};
		...
		ngGridDirectives.directive('ngGrid', ['$compile', '$filter', '$templateCache', '$sortService', '$domUtilityService', '$utilityService', '$timeout',
		function($compile, $filter, $templateCache, sortService, domUtilityService, $utils, $timeout) {
			var ngGrid = {
				scope : true,
				compile : function() {
					return {
						pre : function($scope, iElement, iAttrs) {
							...
							options.gridDim = new ng.Dimension({
							...
							angular.forEach(options.plugins, function(p) {
								if ( typeof p === 'function') {
									p.call(this, []).init($scope.$new(), grid, {
		 ...
		 */
		ngGridDirectives.directive('ngIf', [
		function() {
			return {
				transclude : 'element',
				priority : 1000,
		...
		ngGridDirectives.directive('ngInput', ['$parse',
		function($parse) {
		...
		function($templateCache) {
			...
			$templateCache.put("aggregateTemplate.html", "<div ng-click=\"row.toggleExpand()\" ng-style=\"{'left': row.offsetleft}\" class=\"ngAggregate\">" + "    <span class=\"ngAggregateText\">{{row.label CUSTOM_FILTERS}} ({{row.totalChildren()}} {{AggItemsLabel}})</span>" + "    <div class=\"{{row.aggClass()}}\"></div>" + "</div>" + "");

			$templateCache.put("cellEditTemplate.html", "<div ng-cell-has-focus ng-dblclick=\"editCell()\">" + "	<div ng-if=\"!isFocused\">" + "	DISPLAY_CELL_TEMPLATE" + "	</div>" + "	<div ng-if=\"isFocused\">" + "	EDITABLE_CELL_TEMPLATE" + "	</div>" + "</div>");
			...
			$templateCache.put("checkboxHeaderTemplate.html", "<input class=\"ngSelectionHeader\" type=\"checkbox\" ng-show=\"multiSelect\" ng-model=\"allSelected\" ng-change=\"toggleSelectAll(allSelected)\"/>");

			$templateCache.put("editableCellTemplate.html", "<input ng-class=\"'colt' + col.index\" ng-input=\"COL_FIELD\" />");
			...
			$templateCache.put("headerRowTemplate.html", "<div ng-style=\"{'z-index': col.zIndex(), height: col.headerRowHeight}\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngHeaderCell\" ng-header-cell></div>");

			$templateCache.put("rowTemplate.html", "<div ng-style=\"{'cursor': row.cursor, 'z-index': col.zIndex() }\" ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\" ng-cell></div>");
